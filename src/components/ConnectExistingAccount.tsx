...
interface ConnectExistingAccountProps {
  onBack: () => void;
  onAccountConnected: (profile: FreelancerProfile, collections?: any) => void;
...
export default function ConnectExistingAccount({
  ...
  const finishConnection = useCallback(
    (profile: FreelancerProfile, deviceId?: string, collections?: any) => {
      setIsLoading(false);
      ...
      // Pre-seed local cache with incoming cloud collections
      if (collections && typeof collections === "object") {
        Object.entries(collections).forEach(([col, items]) => {
          if (Array.isArray(items)) {
            localStorage.setItem(`crm_${col}_${completeProfile.id}`, JSON.stringify(items));
          }
        });
      }
      ...
      setTimeout(() => {
        onAccountConnected(completeProfile, collections);
      }, 400);
    },
    [onAccountConnected]
  );
  ...
  const handleEmailSignIn = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok && data.success && data.profile) {
        ...
        finishConnection(data.profile, deviceId, data.collections);
  ...
  // 3. Verify 6-Digit Manual Code OR Workspace / Account ID
  const handleManualCodeSubmit = async (e: React.FormEvent) => {
    ...
    if (!trimmedInput) {
      setErrorMsg("Please enter a connection code or Workspace Account ID.");
      return;
    }

    const cleanNumeric = trimmedInput.replace(/\D/g, "");
    const is6DigitCode = cleanNumeric.length === 6;
    ...
    try {
      ...
      if (is6DigitCode) {
        ...
        // Workspace ID / Account ID direct connection
        res = await fetch("/api/workspace/connect-by-id", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
            workspaceId: trimmedInput,
            clientDeviceId,
            deviceName,
            platform,
            userAgent,
      ...
      if (data.success && data.profile) {
        finishConnection(data.profile, data.device?.id, data.collections);
      ...
      } else if (res.status === 404 || data.error === "invalid" || data.error === "workspace_not_found" || data.error === "not_found") {
        setErrorMsg(
          data.message || "Invalid connection code or Workspace ID. Please verify and try again."
        );
      } else {
  ...
  // Instant Connect with Token (for QR scan or deep link)
  const handleConnectWithToken = async () => {
    if (isLoading || isSubmittingRef.current) return;
    if (!pairingToken && !pairingCode) return;
    isSubmittingRef.current = true;
    setIsLoading(true);
    setErrorMsg("");
    setSuccessMsg("");
    ...
    <div className="min-h-screen bg-slate-50 flex flex-col items-center justify-center p-4">
      ...
      >
        ...
        {pairingStatus === "valid" && workspacePreview && !successMsg && (
          <div className="mb-5 p-3.5 rounded-2xl bg-indigo-50/70 border border-indigo-100 space-y-3">
            ...
            <button
              type="button"
              id="instant-connect-device-btn"
              disabled={isLoading}
              onClick={handleConnectWithToken}
              className="w-full py-3 px-4 bg-indigo-600 hover:bg-indigo-700 active:scale-[0.99] text-white font-bold text-xs rounded-xl shadow-md shadow-indigo-600/20 flex items-center justify-center gap-2 cursor-pointer transition-all disabled:opacity-50"
            >
              {isLoading ? (
              ...
              <span>Connect Device to {workspacePreview.businessName || workspacePreview.name || "Workspace"}</span>
            </button>
          </div>
        )}
        ...
        {activeView === "options" && !successMsg && (
          <div className="space-y-3">
            ...
            <button
              ...
              className="w-full py-3.5 px-4 bg-indigo-600 hover:bg-indigo-700 active:scale-[0.99] text-white disabled:opacity-50 rounded-2xl text-xs font-bold transition-all flex items-center justify-center gap-2.5 cursor-pointer shadow-md shadow-indigo-600/20"
            >
              <Mail size={16} />
            ...
            {/* 6-Digit Code or Workspace ID Button */}
            <button
              type="button"
            ...
            >
              ...
              <span>Enter 6-digit code or Workspace ID</span>
            </button>
          </div>
        )}
        ...
        {/* View 3: 6-DIGIT CODE OR WORKSPACE ID FORM */}
          ...
          <form onSubmit={handleManualCodeSubmit} className="space-y-3.5">
            <div>
              ...
              <input
                ...
                placeholder="e.g. 482910 or 54395c83"
                value={manualCodeInput}
                onChange={(e) => {
                  setManualCodeInput(e.target.value);
                  setErrorMsg("");
                }}
                className="w-full text-center font-mono text-base font-bold py-3 px-3 rounded-xl border border-slate-300 focus:outline-none focus:border-indigo-600 focus:ring-1 focus:ring-indigo-600 text-slate-900 bg-white"
              />
              <p className="text-[11px] text-slate-500 mt-1.5 text-center leading-relaxed">
                Open Freelancer CRM on desktop &rarr; Settings &rarr; Mobile App to view your 6-digit code or Account ID.
              </p>
            </div>
            ...
            >
              ...
              <span>{isLoading ? "Connecting…" : "Connect to Workspace"}</span>
            </button>
